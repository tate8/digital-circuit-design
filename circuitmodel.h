...
private:
    ...
    /// @brief Simulates the circuit with given inputs, and returns the result for the NOT gate
    /// @param input1 - The first input
    bool simulateNotGate (bool inputValue);
...
public slots:
    ...
    /// @brief Runs the circuit on a truth table.
    /// Emits runSuccess or runFailure depending on the results
    /// @param expected - An array of 2 booleans representing the expected results
    /// when trying the circuit with inputs (0) and (1).
    void runNot(const QVector<bool>&);

signals:
    /// @brief Emitted when the circuit is changed
    void circuitChanged(QList<Gate*> gates, QList<Wire*> wires);
